...
export default function BrandOnboarding() {
  ...
  const steps = [
    ...
    { id:3, title:'Review & Submit', current: step===3, completed: false },
  ]
  ...
  const saveDraft = async () => {
    ...
    try {
      if (!brand.slug) {
        const s = brand.name.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')
        setBrand(b => ({...b, slug: s}))
      }
      const payload = {
        id: brand.id,
        name: brand.name,
        slug: brand.slug || brand.name.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''),
  ...
  const submitForReview = async () => {
    ...
    try {
      ...
      if (error) throw error
      setSubmitted(true)
      alert('Submitted for review. You will be notified after moderation.')
    } catch (e:any) {
    ...
    <main className="container space-y-6">
      ...
      {step === 1 && (
        <section className="card space-y-3 max-w-2xl">
          ...
          <input className="w-full p-2 rounded bg-black/30 border border-white/10" value={brand.name} onChange={e=>setBrand({...brand, name:e.target.value})} placeholder="Green Thread Co."/>
      ...
      {step === 3 && (
        <section className="card space-y-3 max-w-2xl">
          ...
          <ul className="text-sm opacity-90 list-disc pl-5 space-y-1">
            ...
            <li><b>Slug:</b> {brand.slug || '(auto from name)'}</li>
          </ul>
          ...
          <div className="flex items-center gap-2">
            <button className="btn btn-outline" onClick={saveDraft} disabled={saving}>Save draft</button>
            <button className="btn btn-primary" onClick={submitForReview} disabled={saving || submitted}>Submit for review</button>
          </div>
          {submitted && <p className="text-green-400 text-sm">Submitted ✓</p>}
        </section>
      )}
    </main>
  )
