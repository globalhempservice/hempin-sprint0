...
function MetricPill({
  ...
  const intent =
    trend === 'down' ? 'good' : 'bad'; // ↓ good (green), ↑ bad (red). Feel free to tweak per kind.

  return (
    <span className={`metric-pill is-${intent}`} role="img" aria-label={aria}>
      <Icon /> <span className="txt">{label}</span> <IconTrend dir={trend} />
    </span>
  );
}

/* ---------- per-step glass glyphs ---------- */
  ...
  const lifecycle: Array<{
    ...
    { key: 'seed',    title: 'Seed',    sub: 'Genetics, soil &amp; weather',
      metrics: [
        ...
        { kind: 'water', trend: 'up', label: 'Water',  aria: 'Water reduced' },
    ...
    { key: 'grow',    title: 'Grow',    sub: 'Field data &amp; regen practices',
      metrics: [
        { kind: 'co2',   trend: 'down', label: 'CO₂e',   aria: 'CO₂ reduced' },
        { kind: 'water', trend: 'up', label: 'Water',  aria: 'Water reduced' },
        { kind: 'energy',trend: 'up', label: 'Energy', aria: 'Energy reduced' },
      ]},
    { key: 'harvest', title: 'Harvest', sub: 'Moisture, yield, quality',
      metrics: [
        { kind: 'co2',   trend: 'up', label: 'CO₂e',   aria: 'CO₂ reduced' },
      ...
      <div className="container">
        ...
        <div className="db-rail hemp-panel" aria-label="One graph pipeline">
          <div className="db-aurora">
            <NebulaDivider label="Molecules→Materials→Components→Products→Brands→Markets" compact animate />
          </div>

          <div className="db-marquee" aria-hidden={false}>
            <div className="track">
          ...
          <ol className="db-stages" role="list">
            {STAGES.map((label) => (
              ...
              {lifecycle.map(({ key, title, sub, metrics }) => (
                <li className="db-card" key={key}>
                  ...
                  <div className="text">
                    ...
                    <span className="muted" dangerouslySetInnerHTML={{ __html: sub }} />
                  </div>
